...
class PostsListBlog extends PostsList {
	...
	loadBlogPosts() {
		...
		const featuredRowEl = this.el.querySelector('.posts-list__row-featured');

		if (featuredRowEl) {
			if (this.filterCategorySlug || this.filterTagSlug || this.filterSearch) {
				featuredRowEl.classList.add('hide');
			} else {
				featuredRowEl.classList.remove('hide');
			}
		}
