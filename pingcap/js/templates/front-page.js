...
class TemplateFrontPage {
	constructor(el) {
		...
		this.animationContainerEl = this.el.querySelector('.tmpl-front-page__animation-container');
		this.transitionItemsMobileEl = this.el.querySelector(
			'.tmpl-front-page__transition-items--mobile'
		);
		this.transitionItemsDesktopEl = this.el.querySelector(
			'.tmpl-front-page__transition-items--desktop'
		);
		this.observersCreated = false;
		this.mobileAssetsLoaded = false;
		this.desktopAssetsLoaded = false;

		this.assetUrlBase = this.animationContainerEl.getAttribute('data-asset-url-base') ?? '';

		if (!this.assetUrlBase) {
			console.error(
				'No [data-asset-url-base] attribute present on the animation container element'
			);
			return;
		}
